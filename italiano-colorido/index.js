...
let principais = document.getElementById('principais')
...
fetch("db.json")
    .then(res => res.json()).then(
        res => {
            res.entradas.map(
                data => {
                    entradas.append(criarDivEntradas(data.id, data.nome, data.valor, data.descricao, data.logo, data.imgs, 'principal'))
                }
            )
            res.pratoPrincipal.map(
                data => {
                    principais.append(criarDivEntradas(data.id, data.nome, data.valor, data.descricao, data.logo, data.imgs))
                }
            )
            res.sobremesas.map(
                data => {
                    sobremesa.append(criarDivEntradas(data.id, data.nome, data.valor, data.descricao, data.logo, data.imgs))
                }
            )
    
            res.vinhos.map(
                data => {
                    vinhos.append(criarDivEntradas(data.id, data.nome, data.valor, data.descricao, data.logo, data.imgs))
                }
            )
            res.semAlcool.map(
                data => {
     ...
     if (id == 1 && principal ) {
         ...
         div.innerHTML += `
              <div class="w-full h-1/2 flex justify-center relative " >
                 <img src="restaurante_italiano/icones/estilos/clique.png" class=" w-10 sm:w-20 h-auto absolute  left-85 sm:left-85 top-60 sm:top-100  animate-fadeInPointer" id="elemento2"  alt="">
             </div>
         `
     }
      ...
      <main class="modal-content  h-auto flex flex-col justify-between bg-white border-b-2">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-32 relative border-b-2 border-b-white py-4">
                <div class="w-full  h-72 sm:h-113  flex flex-col  justify-center items-center sm:justify-start sm:items-start">
                    ...
                    <img src="restaurante_italiano/icones/estilos/linha_italia.jpg" class=" w-56 mt-2 sm:mt-10 " alt="">
                </div>
                <div class="grid grid-cols-2 gap-4 sm:gap-0 sm:grid-cols-1 h-auto ">
                    <div class=" w-full sm:absolute flex justify-between  flex-col left-30 md:left-28 lg:left-30  top-16 ">
                        ...
                        <div class="w-auto  sm:w-56 flex justify-center sm:justify-end sm:mb-16 sm:ml-1">
                            <img src="restaurante_italiano/icones/estilos/bandeira_italia.jpg" class="sm:w-28 w-24 " alt="">
                        </div>
                    </div>
